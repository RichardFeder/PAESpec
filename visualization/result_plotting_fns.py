...
def plot_zscore_vs_multiple_R(sources_data):
    """
    Computes the multiple correlation coefficient (R) for each source's posterior
    samples and plots the redshift z-score against it.

    Args:
        sources_data (dict): A dictionary where each key is a source identifier
                             and the value is a dictionary containing the
                             posterior samples for 'redshift', 'latents', and
                             a 'z_score' for the source.
                             - 'redshift': a 1D numpy array of posterior samples.
                             - 'latents': a 2D numpy array of shape (n_samples, 5).
                             - 'z_score': a single float representing the z-score
                                          for the source.
    """
    all_z_scores = []
    all_R_values = []
    ...
    for source_id, data in sources_data.items():
        redshift_samples = data['redshift']
        latent_samples = data['latents']
        source_z_score = data['z_score']
        
        # Check if there are enough samples to perform regression
        if len(redshift_samples) > 5:
            # Reshape the redshift data to fit the scikit-learn format
            redshift_reshaped = redshift_samples.reshape(-1, 1)

            # Fit the model to the posterior samples for this source
            model.fit(latent_samples, redshift_reshaped)

            # Calculate R^2 and then the multiple correlation coefficient R
            r_squared = model.score(latent_samples, redshift_reshaped)
            multiple_R = np.sqrt(r_squared)
            
            # Store the results
            all_z_scores.append(source_z_score)
            all_R_values.append(multiple_R)

    # Plot the final results
    plt.figure(figsize=(8, 6))
    plt.scatter(all_R_values, all_z_scores, alpha=0.5, color='blue')
    plt.axhline(0, color='k', linestyle='--', linewidth=1)
    
    plt.xlabel('Multiple Correlation Coefficient ($R$)')
    plt.ylabel('Redshift Z-score')
    plt.title('Z-score vs. Multiple Correlation with Latent Variables')
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show()
...
def plot_u_post_marg(u_samples, figsize=(7, 5)):

    latent_dim = u_samples.shape[1]

    if latent_dim > 4 and latent_dim < 7:
        nrow, ncol = 2, 3
    elif latent_dim > 6 and latent_dim < 10:
        nrow, ncol = 3, 3
    else:
        nrow, ncol = 2, 2
    ...
    for n in range(latent_dim):
        med_z = np.median(u_samples[:,n])
        hipct, lopct = np.percentile(u_samples[:,n], 84), np.percentile(u_samples[:,n], 16)
        hisig, losig = hipct-med_z, med_z-lopct
        plt.subplot(nrow, ncol, n+1)
        plt.title('$u_'+str(n)+'='+str(np.round(med_z, 2))+'^{+'+str(np.round(hisig, 2))+'}_{-'+str(np.round(losig, 2))+'}$')
        plt.hist(u_samples[:,n], bins=30, histtype='step')
        plt.axvline(med_z, linestyle='dashed')
    plt.tight_layout()
    plt.show()
    return fig


def plot_logp_trace_indiv(log_pz_chain, logL_chain, log_pz_redshift_chain=None, figsize=(6,3), ncol = 2, lab_fs=14):


    # print(figsize)
    fig_logp_trace = plt.figure(figsize=figsize)

    plt.subplot(1,ncol,1)
    plt.plot(np.arange(len(log_pz_chain)), log_pz_chain)
    plt.ylabel('log $p(u)$', fontsize=lab_fs)
    plt.xlabel('Sample index', fontsize=lab_fs)

    plt.subplot(1,ncol,2)
    plt.plot(np.arange(len(logL_chain)), logL_chain)
    plt.ylabel('log $p(x|u)$', fontsize=lab_fs)
    plt.xlabel('Sample index', fontsize=lab_fs)
...
def plot_redshift_trace(redshift_samp, figsize=(5, 3), lab_fs=12):

    fig = plt.figure(figsize=(5, 3))
    plt.plot(np.arange(len(redshift_samp)), redshift_samp)
    plt.xlabel('Sample index', fontsize=lab_fs)
    plt.ylabel('Redshift', fontsize=lab_fs)
    plt.show()
    return fig
    
def trace_plot(u_samples, figsize=(10, 4)):

    nsamp, nlatent = u_samples.shape[0], u_samples.shape[1]
    fig = plt.figure(figsize=figsize)

    for x in range(nlatent):
        plt.plot(np.arange(nsamp), u_samples[:,x]-np.mean(u_samples[:,x]), color='C'+str(x), label='$u_'+str(x)+'$')
    plt.xlabel('Sample index')
    plt.ylabel('$\\hat{u}-\\overline{u}$', fontsize=14)
    plt.legend()
    plt.show()
...
def compare_coverage(save_fpaths, sample_fpaths, labels=None, nsrc=100, snrstr='25SNR50', filts_str='sphx102', nf_alpha=1, load_tf=False, src_idxs=None, \
                                 tf_results=None, plot_zscore=True, colors=None, dz_oneplusz_max=None, \
                    pitstring = '$z<22.5$'):
    ...
    if load_tf:
        ...
        zpdf_norm /= np.sum(zpdf_norm, axis=1)[:, np.newaxis]
        print("\nNormalized PDF array:\n", zpdf_norm)
        print("\nNew PDF sums (axis=1):", np.sum(zpdf_norm, axis=1))
        # zpdf_norm /= np.sum(zpdf_norm, axis=1)
        
        zscore_tf = ((zout_tf - match_z)/dzout_tf)[which_idxs]
        ztrue_tf = match_z[which_idxs]
        zout_tf = zout_tf[which_idxs]

        print('before cut, there are ', len(zout_tf), ' sources for TF')

        if dz_oneplusz_max is not None:
            tf_mask = (dz_oneplusz_tf[which_idxs] < dz_oneplusz_max)

            ztrue_tf = ztrue_tf[tf_mask]
            zpdf_norm = zpdf_norm[tf_mask]
            zscore_tf = zscore_tf[tf_mask]
            zout_tf = zout_tf[tf_mask]

            print('after cut, there are ', len(zout_tf), ' sources left in TF')
        
        pit_values_tf = compute_pit_values_tf(ztrue_tf, finez, zpdf_norm)
        all_pit_vals.append(pit_values_tf)

        all_zscore.append(zscore_tf)
        all_zout.append(zout_tf)
    ...
    for x in range(len(comb_lists)):
        ...
        dz_oneplusz_pae = dzout_mclmc/(1+redshift_res['z_med'])

        print('before cut, there are ', len(zscore), ' sources for PAE')

        if dz_oneplusz_max is not None:

            pae_mask = (dz_oneplusz_pae < dz_oneplusz_max)
            zscore = zscore[pae_mask]
            zout = zout[pae_mask]
            ztrue = ztrue[pae_mask]
            dzout_mclmc = dzout_mclmc[pae_mask]
            all_samples = all_samples[pae_mask]

            print('after cut, there are ', len(zscore), ' sources left in PAE')
    ...
    if plot_zscore:
        ...
        for k in range(len(all_zscore)):
            plt.hist(all_zscore[k], bins=np.linspace(-5, 5, 50), histtype='step', color=colors[k], label=labels[k], linewidth=1.5, density=True)
            plt.axvline(np.nanmedian(all_zscore[k]), color=colors[k])
        # plt.legend(loc=2, ncol=2, bbox_to_anchor=[-0.1, 1.2])
        plt.xlabel('Redshift z-score', fontsize=12)
        # plt.ylabel('$N_{src}$', fontsize=12)
        plt.grid(alpha=0.3)
        plt.legend(loc=2, ncol=2, bbox_to_anchor=[-0.0, 1.3])
    

        # plt.show()

        dzbins = np.linspace(-0.5, 0.5, 50)
        # dzbins = np.linspace(-0.05, 0.05, 40)
        plt.subplot(2,1,2)
        # plt.figure(figsize=(5, 4))
        for k in range(len(all_zscore)):
            errz_oneplusz = (all_zout[k]-all_ztrue[k])/(1+all_ztrue[k])
            plt.hist(errz_oneplusz, bins=dzbins, color=colors[k], histtype='step', label=labels[k], density=True)
    ...
    if dz_oneplusz_max is not None:
        pitstring += ', $\\hat{\\sigma}_{z/1+z}<$'+str(np.round(dz_oneplusz_max, 2))
    fig = plot_qq(all_pit_vals, pitstring, colors=colors, labels=labels)

    return fig 


def analyze_profile_likelihood_latents_with_nn(profpath, latent_data_path, PAE_obj, 
                                               nf_latents=None, restframe_templates=None,
                                               wavelength_rest=None, pl_file_pattern='*_091725_goodTF.npz',
                                               nlatent=5):
    """
    Analyze profile likelihood latents by finding nearest neighbors in training set 
    and computing MSE between decoder output and corresponding templates.
    
    Parameters:
    -----------
    profpath : str
        Path to profile likelihood files
    latent_data_path : str 
        Path to latent training data (latents.npz)
    PAE_obj : PAE_JAX
        Trained PAE model object
    nf_latents : ndarray, optional
        Normalizing flow latents from training set. If None, will load from latent_data_path
    restframe_templates : ndarray, optional
        Rest frame templates from training. If None, will load from PAE_obj
    wavelength_rest : ndarray, optional
        Rest frame wavelengths. If None, uses PAE_obj wavelengths
    pl_file_pattern : str
        Glob pattern for profile likelihood files
    nlatent : int
    ...
    results : dict
        ...
        - 'profile_latents': Profile likelihood latent parameters
        - 'nearest_neighbors': Indices of nearest neighbors in training set
        - 'nn_distances': Distances to nearest neighbors
        - 'mse_values': MSE between decoder output and templates at each redshift
        - 'redshifts': Redshift grid from profile likelihood
        - 'source_ids': Source IDs from profile likelihood files
    """
    
    # Load profile likelihood files
    import glob
    from sklearn.neighbors import NearestNeighbors
    
    pl_fpaths = glob.glob(profpath + pl_file_pattern)
    print(f"Found {len(pl_fpaths)} profile likelihood files")
    
    # Load training set latents
    if nf_latents is None:
        print(f"Loading training latents from {latent_data_path}")
        latent_file = np.load(latent_data_path)
        
        # Try to load normalizing flow latents (transformed to u-space)
        if 'all_z_train' in latent_file:
            latents_train = latent_file['all_z_train']
            print(f"Loaded training latents shape: {latents_train.shape}")
            
            # Transform to normalizing flow space
            nf_latents = PAE_obj.get_encoded_u(latents_train)
            print(f"Transformed to NF latents shape: {nf_latents.shape}")
        else:
            raise ValueError("Could not find 'all_z_train' in latent file")
    
    # Load rest frame templates if needed
    if restframe_templates is None:
        print("Loading rest frame templates from PAE model...")
        # We'll generate these from the training latents through the decoder
        restframe_templates = PAE_obj.push_spec(latents_train, redshift=0.0)
        print(f"Generated rest frame templates shape: {restframe_templates.shape}")
    
    # Get wavelength grid
    if wavelength_rest is None:
        wavelength_rest = PAE_obj.central_wavelengths_rest
        
    # Set up nearest neighbor search in NF space
    nn_model = NearestNeighbors(n_neighbors=1, metric='euclidean')
    nn_model.fit(nf_latents)
    
    # Process each profile likelihood file
    results = {
        'profile_latents': [],
        'nearest_neighbors': [],
        'nn_distances': [],
    ...
    for i, pl_fpath in enumerate(pl_fpaths):
        print(f"\nProcessing file {i+1}/{len(pl_fpaths)}: {pl_fpath}")
        
        # Load profile likelihood data
        pl_data = np.load(pl_fpath)
        z_grid = pl_data['z_grid']
        all_map_thetas = pl_data['all_map_thetas']  # Shape: (nz, nlatent)
        
        # Extract source ID from filename
        source_id = pl_fpath.split('=')[1].split('_')[0] if '=' in pl_fpath else f"src_{i}"
        
        print(f"  Source ID: {source_id}")
        print(f"  Profile latents shape: {all_map_thetas.shape}")
        print(f"  Redshift grid: {z_grid.min():.2f} - {z_grid.max():.2f}")
        
        # Find nearest neighbors for each redshift's latent parameters
        nn_indices = []
        nn_distances = []
        mse_values = []
        
        for j, (z_val, latent_params) in enumerate(zip(z_grid, all_map_thetas)):
            
            # Find nearest neighbor in training set
            distances, indices = nn_model.kneighbors([latent_params])
            nn_idx = indices[0][0]
            nn_dist = distances[0][0]
            
            nn_indices.append(nn_idx)
            nn_distances.append(nn_dist)
            
            # Get the corresponding rest frame template
            template_restframe = restframe_templates[nn_idx]
            
            # Generate decoder output at this redshift
            decoder_output = PAE_obj.push_spec(latent_params[None, :], redshift=z_val)
            
            # Compute MSE between template and decoder output
            mse = np.mean((template_restframe - decoder_output.flatten())**2)
            mse_values.append(mse)
            
            if j % 20 == 0:  # Print progress every 20 redshifts
                print(f"    z={z_val:.3f}: NN dist={nn_dist:.4f}, MSE={mse:.6f}")
        
        # Store results for this source
        results['profile_latents'].append(all_map_thetas)
        results['nearest_neighbors'].append(np.array(nn_indices))
        results['nn_distances'].append(np.array(nn_distances))
        results['mse_values'].append(np.array(mse_values))
        results['redshifts'].append(z_grid)
        results['source_ids'].append(source_id)
        
        print(f"  Average NN distance: {np.mean(nn_distances):.4f}")
        print(f"  Average MSE: {np.mean(mse_values):.6f}")
    
    return results


def plot_nn_analysis_results(results, save_path=None, figsize=(12, 8)):
    """
    Plot the results from nearest neighbor analysis of profile likelihood latents.
    
    Parameters:
    -----------
    results : dict
        Output from analyze_profile_likelihood_latents_with_nn
    ...
    nsources = len(results['source_ids'])
    
    fig, axes = plt.subplots(2, 2, figsize=figsize)
    fig.suptitle('Profile Likelihood Nearest Neighbor Analysis', fontsize=14)
    
    # Plot 1: Nearest neighbor distances vs redshift
    ax1 = axes[0, 0]
    for i in range(nsources):
        z_grid = results['redshifts'][i]
        nn_distances = results['nn_distances'][i]
        ax1.plot(z_grid, nn_distances, alpha=0.7, label=f"Source {results['source_ids'][i]}")
    
    ax1.set_xlabel('Redshift')
    ax1.set_ylabel('NN Distance in Latent Space')
    ax1.set_title('Nearest Neighbor Distances')
    ax1.grid(True, alpha=0.3)
    if nsources <= 10:
        ax1.legend()
    
    # Plot 2: MSE vs redshift
    ax2 = axes[0, 1]
    for i in range(nsources):
        z_grid = results['redshifts'][i]
        mse_values = results['mse_values'][i]
        ax2.plot(z_grid, mse_values, alpha=0.7, label=f"Source {results['source_ids'][i]}")
    
    ax2.set_xlabel('Redshift')
    ax2.set_ylabel('MSE (Template vs Decoder)')
    ax2.set_title('Template-Decoder MSE')
    ax2.set_yscale('log')
    ax2.grid(True, alpha=0.3)
    
    # Plot 3: Distribution of NN distances
    ax3 = axes[1, 0]
    all_nn_distances = np.concatenate(results['nn_distances'])
    ax3.hist(all_nn_distances, bins=50, alpha=0.7, edgecolor='black')
    ax3.set_xlabel('NN Distance in Latent Space')
    ax3.set_ylabel('Frequency')
    ax3.set_title('Distribution of NN Distances')
    ax3.axvline(np.median(all_nn_distances), color='red', linestyle='--', 
                label=f'Median: {np.median(all_nn_distances):.3f}')
    ax3.legend()
    
    # Plot 4: MSE vs NN distance correlation
    ax4 = axes[1, 1]
    all_mse = np.concatenate(results['mse_values'])
    ax4.scatter(all_nn_distances, all_mse, alpha=0.6, s=20)
    
    # Add correlation coefficient
    corr_coef = np.corrcoef(all_nn_distances, all_mse)[0, 1]
    ax4.set_xlabel('NN Distance in Latent Space')
    ax4.set_ylabel('MSE (Template vs Decoder)')
    ax4.set_title(f'MSE vs NN Distance (r={corr_coef:.3f})')
    ax4.set_yscale('log')
...
def summarize_nn_analysis(results):
    """
    Print summary statistics from nearest neighbor analysis.
    ...
    print("\n" + "="*60)
    print("NEAREST NEIGHBOR ANALYSIS SUMMARY")
    print("="*60)
    ...
    print(f"Number of sources analyzed: {len(results['source_ids'])}")
    print(f"Total profile likelihood evaluations: {len(all_nn_distances)}")
    
    print(f"\nNearest Neighbor Distances:")
    print(f"  Mean: {np.mean(all_nn_distances):.4f}")
    print(f"  Median: {np.median(all_nn_distances):.4f}")
    print(f"  Std: {np.std(all_nn_distances):.4f}")
    print(f"  Range: [{np.min(all_nn_distances):.4f}, {np.max(all_nn_distances):.4f}]")
    
    print(f"\nTemplate-Decoder MSE:")
    print(f"  Mean: {np.mean(all_mse):.6f}")
    print(f"  Median: {np.median(all_mse):.6f}")
    print(f"  Std: {np.std(all_mse):.6f}")
    print(f"  Range: [{np.min(all_mse):.6f}, {np.max(all_mse):.6f}]")
    
    # Correlation
    corr_coef = np.corrcoef(all_nn_distances, all_mse)[0, 1]
    print(f"\nCorrelation between NN distance and MSE: {corr_coef:.4f}")
    
    # Per-source breakdown
    print(f"\nPer-source statistics:")
    print(f"{'Source ID':<12} {'Avg NN Dist':<12} {'Avg MSE':<12} {'N Points':<10}")
    print("-" * 50)
    
    for i, source_id in enumerate(results['source_ids']):
        avg_nn_dist = np.mean(results['nn_distances'][i])
        avg_mse = np.mean(results['mse_values'][i])
        n_points = len(results['nn_distances'][i])
        print(f"{source_id:<12} {avg_nn_dist:<12.4f} {avg_mse:<12.6f} {n_points:<10}")
